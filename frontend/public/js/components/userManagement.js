...
const UserManagementComponent = {
                    ...
                    <tbody>
                        ${UserManagementComponent.users.map(user => `
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="py-4 px-4">
                                    <span class="badge badge-${user.role === 'admin' ? 'primary' :
                user.role === 'technician' ? 'warning' :
                    'success'
            }">
                                        <i class="fas fa-${user.role === 'admin' ? 'user-shield' :
                                ...
                                <td class="py-4 px-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ${user.id !== currentUser.id ? `
                                            <button 
                                                onclick="UserManagementComponent.openPasswordModal('${user.id}', '${user.username}')"
    ...
    openPasswordModal: (userId, username) => {
        const modalHTML = `
            <div id="password-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4">
                    <div class="bg-gradient-to-r from-yellow-600 to-orange-600 p-6 rounded-t-xl">
                        <h2 class="text-2xl font-bold text-white">Şifre Değiştir</h2>
                        <p class="text-yellow-100 mt-1">Kullanıcı: ${username}</p>
                    </div>
                    <form id="password-form" class="p-6" data-user-id="${userId}">
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t">
                            <button type="button" onclick="UserManagementComponent.closePasswordModal()" 
                                class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">İptal</button>
                            <button type="submit" 
                                class="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700">Değiştir</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        document.body.insertAdjacentHTML('beforeend', modalHTML);
        document.getElementById('password-form').addEventListener('submit', UserManagementComponent.handlePasswordChange);
    },
    ...
    handlePasswordChange: async (e) => {
        ...
        try {
            await API.users.updatePassword(userId, newPassword);
            hideLoading();
            showToast('Kullanıcı şifresi değiştirildi', 'success');
            UserManagementComponent.closePasswordModal();
        } catch (error) {
            hideLoading();
            showToast('Hata: ' + error.message, 'error');
        }
